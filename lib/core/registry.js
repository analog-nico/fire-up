...
function newRegistry(fireUp) {
  ...
  registry.findInterface = function (moduleReference) {
    ...
    // TODO: currentInterface does not need to be computed anymore because this string can be retrieve by the registryPointer
    ...
    for ( var i = 0; i < segments.length; i+=1 ) {
      ...
      if (_.isUndefined(registryPointer[segments[i]])) {
        fireUp.log.warn('No implementation found for module reference: ' + moduleReference);
      ...
      if (i === segments.length-1) {
        ...
        if (_.isString(registryPointer[segments[i]].file)) {
          ...
          fireUp.log.warn('No implementation available due to conflicting implementations for the interface: ' + currentInterface);
          return;
        }
        ...
        fireUp.log.warn('No exact or more specific, unambiguous implementation found for module reference: ' + moduleReference);
