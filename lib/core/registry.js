  ...
  registry.findInterface = function (moduleReference) {
    ...
    for ( var i = 0; i < segments.length; i+=1 ) {
      ...
      if (_.isUndefined(registryPointer[segments[i]])) {
        fireUp.log.warn({}, "No implementation found for module reference '%s'.", moduleReference);
    ...
    fireUp.log.warn({}, "No exact or more specific, unambiguous implementation found for module reference '%s'.", moduleReference);
      ...
      if (p === interfaceChain.length-1) {
        ...
        if (_.isNull(registryPointer[interfaceChain[p]].file) === false ||
          ...
          fireUp.log.error({
            ...
            conflictingFiles: registryPointer[interfaceChain[p]].conflictingFiles
          }, "Some files conflict by implementing the same interface '%s'.", currentInterface);
        ...
        fireUp.log.info({
          interfaceName: currentInterface,
          file: filePathRelativeToCwd
        }, 'Registered module at \'%s\' as implementation for interface \'%s\'.', filePathRelativeToCwd, currentInterface);
