...
function newLoader(fireUp) {
  ...
  function loadModule(injectionRequest, options) {
    ...
    // TODO: Add tests that load fireUp interfaces through fireUp(...)
    // TODO: Add tests that replace those interfaces by extended interfaces.
    if (injectionRequest.usedInterface.match(/^fireUp\//m)) {
      if (injectionRequest.usedInterface === 'fireUp/currentInjector') {
        ...
        return Promise.resolve(boxLoadedDependency('fireUp/options', options));
      } else {
