  ...
  function validateModule(module, filePathAbsolute) {
    ...
    if (_.isUndefined(module.__module.inject) === false) {
      ...
      var injectedOwnInterfaces = _.intersection(
          _.isArray(module.__module.implements) ? module.__module.implements : [module.__module.implements],
          _.isArray(module.__module.inject) ? module.__module.inject : [module.__module.inject]
      );
      if (injectedOwnInterfaces.length > 0) {
        ...
        fireUp.log.error({ err: err }, err.message);
        fireUp.log.debug({ injectedOwnInterfaces: injectedOwnInterfaces }, "Please make sure that __module.implements and __module.inject are distinct.");
        throw err;
      }
  ...
  function injectionMovesInCircles(injectionRequest) {
    ...
    if (injectionRequest._internal.moduleRecord.type === fireUp.constants.MODULE_TYPE_SINGLETON) {
      ...
      while (_.isUndefined(tempInjectionRequest) === false) {
        if (tempInjectionRequest._internal.interfaceRecord.file === injectionRequest._internal.interfaceRecord.file) {
          return true;
        }
