...
function newLoader(fireUp) {
  ...
  function validateModule(module, filePathAbsolute) {
    ...
    if (_.isFunction(module) === false) {
      fireUp.log.error({}, "The following module did not export a function: %s", filePathAbsolute);
      fireUp.log.debug({}, "Please make sure that the module follows the factory pattern.");
      return false;
    }

    if (_.isUndefined(module.__module)) {
      fireUp.log.error({}, "The following module does not export the __module property: %s", filePathAbsolute);
      ...
      fireUp.log.error({}, "The following module does not export an object for the __module property: %s", filePathAbsolute);
      fireUp.log.debug({}, "At minimum the following statement is required: module.exports = { implements: '<module interface>' };");
      return false;
    }

    if (_.isUndefined(module.__module.implements)) {
      fireUp.log.error({}, "The following module does not export the __module.implements property: %s", filePathAbsolute);
      ...
      fireUp.log.error({}, "The following module exports the __module.implements property in the wrong format: %s", filePathAbsolute);
      fireUp.log.debug({}, "You may assign a string denoting the interface implemented by the module or an array of strings for multiple interfaces.");
      return false;
      ...
      for ( i = 0; i < module.__module.implements.length; i+=1 ) {
        if (_.isString(module.__module.implements[i]) === false ||
          ...
          fireUp.log.error({}, "The following module exports an invalid interface name at index %d of the __module.implements property: %s", i, filePathAbsolute);
          fireUp.log.debug({}, 'A valid interface name must not contain whitespaces nor parenthesises.');
          return false;
        }
      }
      ...
      if (_.uniq(module.__module.implements).length < module.__module.implements.length) {
        fireUp.log.error({}, "The following module exports duplicated interface names in the __module.implements property: %s", filePathAbsolute);
        return false;
      }
      ...
      fireUp.log.error({}, "The following module exports an empty module name through the __module.implements property: %s", filePathAbsolute);
      return false;
    }
    ...
    if (_.isUndefined(module.__module.inject) === false) {
      ...
      if (_.isString(module.__module.inject) === false && _.isArray(module.__module.inject) === false) {
        fireUp.log.error({}, "The following module exports the __module.inject property in the wrong format: %s", filePathAbsolute);
        ...
        for ( i = 0; i < module.__module.inject.length; i+=1 ) {
          if (_.isString(module.__module.inject[i]) === false ||
            ...
            fireUp.log.error({}, "The following module exports an invalid module reference at index %d of the __module.inject property: %s", i, filePathAbsolute);
        ...
        fireUp.log.error({}, "The following module exports an invalid module reference through the __module.inject property: %s", filePathAbsolute);
        return false;
      }
    ...
    if (_.isUndefined(module.__module.type) === false) {
      ...
      if (_.isString(module.__module.type) === false ||
        ...
        fireUp.log.error({}, "The following module exports an invalid module type through the __module.type property: %s", filePathAbsolute);
  ...
  function registerModules() {
    ...
    _.forOwn(fireUp._internal.registry.modules, function(registryEntry, modulePath) {
      ...
      for ( var r = 0; r < registryEntry.inject.length; r+=1 ) {
        ...
        var internalDependencies = ['fireUp/currentInjector', 'fireUp/injectionRequest'];
  ...
  function loadModule(moduleReference, parentInjectionRequest) {
    ...
    if (_.isUndefined(interfaceRecord)) {
      fireUp.log.error({}, 'fireUp(\'%s\') failed because no implementation was found for the interface.', moduleReference);
    ...
    var promise = new Promise(function (resolve) {
      ...
      for ( var i = 0; i < moduleRecord.inject.length; i+= 1 ) {
        ...
        promisesForInjectedModules.push(loadModule(moduleRecord.inject[i], injectionRequest));
          ...
          .then(function (modules) {
            return new Promise(function (resolve) {
              ...
              try {
                ...
                var initErr = new fireUp.errors.InstanceInitializationError(moduleReference, interfaceRecord.file, e);
