    ...
    return new Promise(function (resolve) {
      ...
      resolve(Promise.all(promisesForMatchingModules)
          .then(function (modules) {
            var loadedModules = {};
            for ( var i = 0; i < interfaceKeys.length; i+=1 ) {
              loadedModules[baseInterface.interfaces[interfaceKeys[i]].interfaceName] = modules[i];
            }
  ...
  function loadModule(injectionRequest, options) {
    ...
    var promise = new Promise(function (resolve) {
      ...
      for ( var i = 0; i < moduleRecord.inject.length; i+= 1 ) {
        ...
        promisesForInjectedModules.push(loadDependency(moduleRecord.inject[i], options, injectionRequest));
