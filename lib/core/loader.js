...
function newLoader(fireUp) {
  ...
  function validateUseOptions(options) {
    ...
    for ( var i = 0; i < options.use.length; i+=1 ) {
      ...
      if (parsedInterfaceName.segments.length === 1) {
        ...
        fireUp.log.error({ err: err }, err.message);
        fireUp.log.debug({}, 'options.use is designed to replace an implementation of an interface with a more specific one. E.g. for unit testing \'%s:mock\' would make sense.', parsedInterfaceName.segments[0]);
        throw err;
      }
  ...
  function validateModule(module, filePathAbsolute) {
    ...
    if (_.isArray(module.__module.implements)) {
      for ( i = 0; i < module.__module.implements.length; i+=1 ) {
        if (_.isString(module.__module.implements[i]) === false ||
          ...
          fireUp.log.error({}, "The following module exports an invalid module name at index %d of the __module.implements property: %s", i, filePathAbsolute);
          return false;
        }
      }
    ...
    if (_.isObject(module.__module.inject)) {
      ...
      if (_.isString(module.__module.inject) === false && _.isArray(module.__module.inject) === false) {
        fireUp.log.error({}, "The following module does not export the __module.inject property: %s", filePathAbsolute);
        return false;
      }
      ...
      if (_.isArray(module.__module.inject)) {
        for ( i = 0; i < module.__module.inject.length; i+=1 ) {
          if (_.isString(module.__module.inject[i]) === false ||
            ...
            fireUp.log.error({}, "The following module exports an invalid module name at index %d of the __module.inject property: %s", i, filePathAbsolute);
            return false;
          }
        }
        ...
        fireUp.log.error({}, "The following module exports an empty module name through the __module.inject property: %s", filePathAbsolute);
        return false;
      }
        ...
        for ( var p = 0; p < interfaceChain.length; p+=1 ) {
          ...
          if (p === interfaceChain.length-1) {
            if (_.isPlainObject(registryPointer[interfaceChain[p]]) &&
              ...
              fireUp.log.error({}, "The following files conflict by implement the same '%s' interface:", currentInterface);
