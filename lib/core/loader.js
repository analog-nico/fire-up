...
function newLoader(_fireUp) {
  ...
  function registerModules() {
    ...
    for ( var i = 0; i < files.length; i+=1 ) {
      ...
      fireUp._internal.registry.files[filePath].status = fireUp.constants.FILE_STATUS_UNCHECKED;
      ...
      if (validateModule(module, filePath) === false) {
        fireUp._internal.registry.files[filePath].status = fireUp.constants.FILE_STATUS_INVALID;
        continue;
      }
      fireUp._internal.registry.files[filePath].cache = module;
      fireUp._internal.registry.files[filePath].status = fireUp.constants.FILE_STATUS_LOADED;
      ...
      fireUp._internal.registry.files[filePath].status = fireUp.constants.FILE_STATUS_REGISTERED;
