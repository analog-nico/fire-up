...
function newLoader(fireUp) {
  ...
  function registerModulesFromPaths(modulePaths) {
    ...
    for (var i = 0; i < files.length; i += 1) {
      ...
      validateAndRegisterLoadedModule(module, filePathAbsolute, filePathRelativeToCwd);
  ...
  function registerCustomModules(customModules) {
    ...
    for ( var i = 0; i < customModules.length; i+=1 ) {
      ...
      validateAndRegisterLoadedModule(customModules[i], registryId, registryId);

    }

  }

  function validateAndRegisterLoadedModule(loadedModule, filePathAbsolute, filePathRelativeToCwd) {

    try {

      validateModule(loadedModule, filePathAbsolute);
    ...
    fireUp._internal.registry.modules[filePathRelativeToCwd].cache.module = loadedModule;
    if (_.isString(loadedModule.implements)) {
      fireUp._internal.registry.modules[filePathRelativeToCwd].implements = [loadedModule.implements];
    } else {
      fireUp._internal.registry.modules[filePathRelativeToCwd].implements = loadedModule.implements;
    }
    if (_.isUndefined(loadedModule.inject) === false) {
      if (_.isString(loadedModule.inject)) {
        fireUp._internal.registry.modules[filePathRelativeToCwd].inject = [loadedModule.inject];
      } else {
        fireUp._internal.registry.modules[filePathRelativeToCwd].inject = loadedModule.inject;
      }
    }
    if (_.isString(loadedModule.type)) {
      fireUp._internal.registry.modules[filePathRelativeToCwd].type = loadedModule.type;
    }
    fireUp._internal.registry.modules[filePathRelativeToCwd].status = fireUp.constants.MODULE_STATUS_LOADED;
    ...
    fireUp._internal.registry.modules[filePathRelativeToCwd].status = fireUp.constants.MODULE_STATUS_REGISTERED;
